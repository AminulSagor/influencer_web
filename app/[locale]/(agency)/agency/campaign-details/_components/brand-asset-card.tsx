...
function formatFileSize(size?: string) {
  ...
  if (!Number.isFinite(bytes) || bytes <= 0) return size || "Link";
...
const BrandAssetCard = ({ assets }: BrandAssetCardProps) => {
  return (
    <Card className="h-full">
      ...
      <CardContent className="space-y-2">
        {assets.length ? (
          ...
          <p className="text-sm text-muted-foreground">No brand assets available</p>
        )}
      </CardContent>
    </Card>
  );
