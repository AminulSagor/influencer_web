              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-3">
                  <Button
                    ...
                    disabled={isLoading || niche.status === "Rejected"}
                    onClick={() => {
                      setSelectedName(niche.name);
                      setRejectOpen(true);
                    }}
                  >
                    {isLoading && loadingName === niche.name ? "Please wait..." : niche.status === "Rejected" ? "Rejected" : "Reject"}
                  </Button>
                  ...
                  <Button
                    ...
                    variant="lightGreen"
                    className={`min-w-[108px] rounded-2xl ${
                      niche.status === "Accepted"
                        ? "bg-[#e8f8ee] text-[#078834] hover:bg-[#e8f8ee]/90"
                        : "bg-[#86a857] text-white hover:bg-[#78994d]"
                    }`}
                    disabled={isLoading || niche.status === "Accepted"}
                    onClick={() => handleApprove(niche.name)}
                  >
                    {isLoading && loadingName === niche.name ? "Please wait..." : niche.status === "Accepted" ? "Approved" : "Approve"}
                  </Button>
                </div>
              </div>
            );
